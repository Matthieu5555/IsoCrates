...
def add_version_history_to_doc(doc_path: Path, doc_id: str):
    ...
    if not bottomatter_match:
        logger.warning("No bottom matter found in %s", doc_path)
        return
    ...
    if not version_section:
        logger.info("No version history found for %s", doc_id)
        return
    ...
    logger.info("Added version history to: %s", doc_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        logger.info("Usage: python add_version_history.py <doc_path> <doc_id>")
        sys.exit(1)
